...
//      }
  ...
  public String insertPost(Post post, Model model) {
    // 01-1. 파일첨부
    if(post.getUploadingFile() != null && post.getUploadingFile().getOriginalFilename().equals("") == false) {
      String uploadedFile = this.uploadFile(post.getUploadingFile(), post.getName());
      post.setAttachedFile(uploadedFile);
    } else {
      post.setAttachedFile("");
    }
  ...
  public ResponseEntity<Resource> downloadFile(@PathVariable String fileName, HttpServletRequest request) throws Exception {
    //
    File downloadFile = new File("/home/ubuntu/upload/" + fileName);
  ...
  private String uploadFile(MultipartFile file, String uploaderName) {
    //
    String currentWorkingDirectory = "/home/ubuntu";
